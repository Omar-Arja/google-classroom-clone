...
try {
    $query = $mysqli->prepare('insert into submissions (user_id, assignment_id,submission_date, file_path) values (?, ?,now(),?)');
    $query->bind_param('iis', $user_id, $assignment_id, $target_path);
    $query->execute();
